...
\medskip
 ...
 \begin{equation}
x(\;) \Longleftrightarrow \forall t \in \mathbb{R} :\ x(t) = s
\label{eq:time-cont-signal}
\end{equation}


\begin{equation}
x[\;] \Longleftrightarrow \forall n \in \mathbb{Z} :\ x[n] = s
...
\begin{equation}
\begin{gathered} 
x_1[\;] + x_2[\;] = y[\;]\Longleftrightarrow \mathop{\forall}_{n = n_1}^{n_2}  :  x_1[n] + x_2[n] = y[n] \\
x_1[\;] \cdot x_2[\;] = y[\;]\Longleftrightarrow \mathop{\forall}_{n = n_1}^{n_2}  :  x_1[n] \cdot x_2[n] = y[n]
\end{gathered}
\label{eq:addAndMult}
\end{equation}
...
\begin{equation}
x'[\;] \Longleftrightarrow \mathop{\forall}_{n = n_1}^{n_2} :\ x'[n] = x[n] + e[n]
\label{eq:sigErrorAddition}
\end{equation}

\begin{equation}
e[\;] \Longleftrightarrow \mathop{\forall}_{n = n_1}^{n_2} :\ e[n] = x'[n] -x[n]
...
\begin{equation}
r[\;] \Longleftrightarrow \mathop{\forall}_{k = 0}^{k_{max}} :\ r[k] = \text{X-Corr}(x[\;],y[\;],k)
...
\end{equation}
 ...
 \begin{equation}
x[\;] *  h[\;] =  y[\;] \Longleftrightarrow \mathop{\forall}_{n = 0}^{N+M-2} :\ x[n] * h[n] = y[n]
 \label{eq:convolutionSum}
 \end{equation}
...
\begin{equation}
\mathop{\forall}_{n = -\infty}^{\infty} :\ \delta[n] = 
\begin{cases}
1 \quad , n = 0\\
0 \quad ,  n \neq 0
\label{eq:delta}
...
\begin{split}
x[\;] \Longleftrightarrow \mathop{\forall}_{n = 0}^{N-1} :\ x[n] =  A[0]\cos_{f_0}[n] + \ldots + A[N/2]\cos_{f_{N/2}}[n]  \\ + B[0]\sin_{f_{N/2}}[n] + \ldots + B[N/2] \sin_{f_{N/2}}[n]
\end{split}
\end{equation}
...
\begin{equation}
\text{DFT}\{x[\;]\} = X[\;] \Longleftrightarrow
\begin{dcases}
\mathop{\forall}_{k = 0}^{N/2} :\ \bar{A}[k] = \sum_{n=0}^{N-1}x[n] \cos(2\pi k \frac{n}{N}) \Longleftrightarrow \bar{A}[\;] \\
\mathop{\forall}_{k = 0}^{N/2} :\ \bar{B}[k] = \sum_{n=0}^{N-1}x[n] \sin(2\pi k \frac{n}{N}) \Longleftrightarrow \bar{B}[\;] 
\end{dcases}
\label{eq:forwardDFT}
\end{equation}
...
\begin{equation}
A[\;] \Longleftrightarrow \mathop{\forall}_{k = 0}^{N/2} :\ A[k] = 
\begin{dcases}
...
\begin{equation}
B[\;] \Longleftrightarrow \mathop{\forall}_{k = 0}^{N/2} :\
B[k]= \frac{B_k}{N/2}
...
\begin{equation}
\text{DFT}\{X[\;]\} = x[\;]
\Longleftrightarrow \mathop{\forall}_{n = 0}^{N-1} :\ x[n] = \sum_{k = 0}^{N/2} A[k]\cos(2\pi k \frac{n}{N}) + \sum_{k = 0}^{N/2}B[k]\sin(2\pi k \frac{n}{N}) \\
\label{eq:inverseDFT}
\end{equation}
...
\begin{dcases}
\mathop{\forall}_{k = 0}^{N/2} :\ M[k] = \sqrt{(A[k]^2 + B[k]^2 ) } \Longleftrightarrow M[\;]  \\
\mathop{\forall}_{k = 0}^{N/2} :\ \phi[k]= \arctan{(B[k] / A[k]) } \Longleftrightarrow \phi[\;]  
\end{dcases}
...
\begin{dcases}
\mathop{\forall}_{k = 0}^{N/2} :\ A_k= M_k \cdot \cos(\phi_k) \Longleftrightarrow A[\;]  \\
\mathop{\forall}_{k = 0}^{N/2} :\ B_k = M_k \cdot \sin(\phi_k) \Longleftrightarrow B[\;]  
\end{dcases}
...
\begin{equation}
\begin{gathered}
\cos_k[\;] \Longleftrightarrow \mathop{\forall}_{n = 0}^{N-1} :\  \Re(e^{i\cdot 2\pi k \frac{n}{N}}) = \cos_k[n] = \cos(2\pi k \frac{n}{N}) \\
\sin_k[\;] \Longleftrightarrow \mathop{\forall}_{n = 0}^{N-1} :\ \Im(e^{i\cdot 2\pi k \frac{n}{N}}) = \sin_k[n] = \sin(2\pi k \frac{n}{N})
\end{gathered}
...
DFT(x,k) = X[k] = \frac{1}{N} \sum_{n = 0}^{N-1}  x[n] \cdot (\cos (2\pi k \frac{n}{N}) -i \sin (2 \pi k \frac{n}{N}) )
\end{equation}

\begin{equation}
\label{eq:complexDFTpolar}
DFT(x,k) = X[k] =  \frac{1}{N} \sum_{n = 0}^{N-1}  x[n] \cdot e^{-i 2\pi k \frac{n}{N}}
\end{equation}

Gleichung \ref{eq:dftWholeSignal} verdeutlicht die Zusammensetzung des gesamten Signales $X$.

\begin{equation}
DFT\{x\} = X = \big[ DFT(x,0) \ldots DFT(x,N-1)]
\label{eq:dftWholeSignal}
\end{equation}

Die wichtigen Unterschiede zwischen den komplexen DFT nach Formel  \ref{eq:complexDFTrectanulgar} und der reellen DFT nach Formel \ref{eq:forwardDFT} Formelsind (a) die Verwendung des Sinus als komplexe Zahl $i \cdot \sin$, sowie das invertieren seines Vorzeichens und (b) die Summierung über $n = 0\ldots N-1 $ anstatt $n = 0 \ldots N/2$. Der Frequenz-Bereich wird nun durch das complexe Signal $X$ ausgedrückt, welcher die Koeffizienten $A,B$ in seinem Real/Imaginärteil speichert. Der Frequenz-Bereich $X$ hat die selbe Länge wie der Zeit-Bereich $x$, das heißt $\text{Length}(x) = \text{Length}(X)$. \cite[S. 571]{dspGuide}
...
Gleichung \ref{eq:inverseComplexDFTpolar} definiert die dazu inverse Operation, die \emph{komplexe inverse DFT} (iDFT) in \emph{polrarer Notation}. \cite[S. 572]{dspGuide} Gleichung \ref{eq:idftWholeSignal} verdeutlicht die Zusammensetzung des gesamten Signals $x$ auf Basis von Gleichung \ref{eq:inverseComplexDFTpolar}.

\begin{equation}
\label{eq:inverseComplexDFTpolar}
iDFT(X,n) = x[n] =  \sum_{k = 0}^{N-1}  X[k] \cdot e^{i 2\pi k \frac{n}{N}}
\end{equation}

\begin{equation}
iDFT\{X\} = x = \big[ \: [ iDFT(X,0) ,\ldots, iDFT(X,N-1)]
...
Es ist wünschenswert, einen Kompromiss aus den Vorteilen beider Bereiche zu finden, in dem man das Frequenz-Spectrum kürzerer Zeitabschnitte des Signals bildet. Dazu wird der Zeit-Bereich $x$ in Fenster der Länge $M$ zerlegt. Die zeitliche Differenz zwischen zwei Fensterns wird als \emph{Hoptime} $R$ bezeichnet. Gleichung definiert die Bildung des Signal-Fensters $x_m$.\cite{juliusSmith}
 ...
 \begin{equation}
x_{m}[n] = x[n+m\cdot R] \quad , n = 0 ,\ldots, M-1 
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.8\textwidth]{bilder/signalWindows.png}
	\caption{Zerlegung eines Signals in Singal-Fenster}
	\label{img:siganlWindows}
\end{figure}

Als Vorbereitungsschritt für die Transformation der Signal-Fenster in den Frequenz-Bereich wird nun jedes Fenster mit einer sogenannten \emph{Fensterfunktion} (engl \emph{window}) $w[n]$ multipliziert. Gleichung \ref{eq:hammingWindow} definiert eine der am weitesten verbreiteten Fenster-Funktionen, das \emph{Hamming-Window}. Der Paramter $M$ gibt die länge des Fensters an. Abbildung 	\ref{img:hamming} visualisiert das Hamming-Window. \cite[S. 286]{dspGuide}

 \begin{equation}
w[n] = 0.54 - 0.46 \cos(\frac{2\pi n}{M} ) \quad, n = 0 ,\ldots, M-1 
...
Die Gleichung \ref{eq:stft} definiert die \emph{Kurzzeit-Fourier-Transformation} (engl \emph{Short Time Fourier Transformation}, \emph{STFT}), implementiert mit Hilfe der DFT. Dabei wird das Signalfenster $x_m$ mit der Fensterfunktion $w$ multipliziert und in das \emph{Frequenz-Fenster} $X_m$ transformiert.\cite{stft} Gleichung \ref{eq:stftWhole} definiet das gesamte Frequenz-Fenster $X_m$. Abbildung \ref{img:stft02} visualisiert die STFT des Beispiels aus Abbildung \ref{img:siganlWindows}.

 \begin{equation}
STFT(x_m,k) = X_m[k] = \sum_{n=0}^{M-1} x[n+m R] \cdot w[n] \cdot e^{-i 2\pi k \frac{n}{N}}
\label{eq:stft}
\end{equation}

 \begin{equation}
STFT\{x_m\} = X_m = DFT\{x_m \cdot w\}
\label{eq:stftWhole}
\end{equation}

 \begin{figure}[h]
	\centering
	\includegraphics[width=1\textwidth]{bilder/stft02.png}
...
\item $S\{ x[n] \} = y[n]$ definiert das System als Operation $S\{\;\}$. Angewandt auf das gesamte Signal gilt $S\{ x \} = \big[ S\{ x[0] \} \ldots S\{ x[N-1] \} \big] $. \cite[]{dspMichiganSystems}
\end{itemize}
