\documentclass[twoside, openright, bibliography=totoc, listof=totoc, index=totoc, numbers=noenddot, enabledeprecatedfontcommands]{scrreprt}
\usepackage[ngerman]{babel}
\usepackage[utf8]{inputenc}
\setlength{\parskip}{5.5pt}
\setlength{\parindent}{1em}
\usepackage{hyperref}
\usepackage{amssymb,amsmath,amsthm,enumitem}
\usepackage{mathtools}
\usepackage{algpseudocode}
\usepackage{algorithm}
\usepackage[square,sort,comma,numbers]{natbib}
%\usepackage{biblatex}
%\usepackage{bibtex}
\usepackage{graphicx}
\usepackage[normalem]{ulem}
\usepackage{soul}
\usepackage{color}
\usepackage[table,xcdraw]{xcolor}
\usepackage{fancyhdr}
%\usepackage{scrlayer-scrpage}
\usepackage{amsfonts}
\usepackage{booktabs}
\usepackage{siunitx}
\usepackage{longtable}
\usepackage{enumitem}
\usepackage{pdflscape}
\usepackage{listings} 
\usepackage[toc,page]{appendix}
\usepackage{geometry}
\usepackage{multirow}
\usepackage[T1]{fontenc}% wichtig für Trennung von Wörtern mit Umlauten
\usepackage{microtype}% verbesserter Randausgleich


\DeclareMathOperator*{\argmax}{arg\,max}
\DeclareMathOperator*{\maxi}{max}
\DeclareMathOperator*{\maxmagi}{max mag}
\DeclareMathOperator*{\mini}{min}
\DeclareMathOperator*{\counti}{count-peaks}
\DeclareMathOperator*{\meani}{mean}
\DeclareMathOperator*{\mediani}{median}
\DeclareMathOperator{\ReX}{ReX}
\DeclareMathOperator{\ImX}{ImX}

\setcounter{secnumdepth}{3}
% design

%\geometry{a4paper,left=30mm,right=30mm, top=30mm, bottom=35mm minus 5mm} 
\geometry{a4paper,left=37mm,right=23mm, top=30mm, bottom=35mm minus 5mm} 

\usepackage[skip=5pt]{caption}
\setlength{\footskip}{15mm}

\pagestyle{fancy}
\fancyhf{}
\chead{\nouppercase{\rightmark}}
\cfoot{\thepage}

\raggedbottom

\widowpenalty10000
%\clubpenalty10000



%\title{Visualisierung kontinuierlicher, multimodaler Schmerz Scores am Beispiel akustischer Signale}
%\subtitle{Masterarbeit}
%\author{Franz Anders \\ HTWK Leipzig }
%\date{Januar 2017}



\begin{document}

\include{chapters/title_page_1}
\newpage\null\thispagestyle{empty}\newpage	

\pagenumbering{Roman} 
\include{chapters/Erklaerung}
\tableofcontents

\include{chapters/abkurz}

\listoffigures
\listoftables
\pagebreak
%\begingroup
%   \let\clearpage\relax        % report
%   \tableofcontents
%   \listoftables
%   \listoffigures
%\endgroup
\pagenumbering{arabic}
%set this one if chapter begins on wrong side
\setcounter{page}{0}
\include{chapters/Introduction} 
\include{chapters/Foundations}
\include{chapters/Konzept}
\include{chapters/VAD}
\include{chapters/Deduction}
\include{chapters/Visualisation}
\include{chapters/Conclusion}

\pagenumbering{Roman}
%set this one if chapter begins on wrong side%\setcounter{page}{0}

%\bibliographystyle{dinat}
\bibliographystyle{alphadin}
\bibliography{references}

\include{chapters/Anhang}


\end{document}
